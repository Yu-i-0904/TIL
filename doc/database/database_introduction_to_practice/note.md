...
## 11.1 インデックスの働き
* RDBのインデックス
  * B+ツリーは、データが格納されたリーフノードとリーフノードへの経路としての役割を持つノンリーフノードから形成される
  * 経路の出発点になるノードをルートノードという
  * ノンリーフノードには、子ノードが保有する値のうちの最小値が格納される
  * B+ツリーインデックスはインデックス内のエントリ数がテーブルの行数と全く同じになる
  * そのため、テーブルの行数が増えればインデックスも大きくなる
* インデックスの左端と範囲検索
  * B+ツリーインデックスは、等価比較および範囲検索に利用できる（完全一致や不等号、BETWEEN、LIKE句の部分一致）
  * インデックスを利用した検索を行うには、ワイルドカードの指定は、前方一致、つまりワイルドカードが具体的な文字の後に来る
  * B+ツリーは、左端の文字から順にソートされているため、左端以外で検索をしても、構造的に不可能になる
  * マルチカラムも同様に左にあるカラムから順に指定する必要がある
* セカンダリインデックス
  * 主キーの値は、基本的に変わることはないが、セカンダリインデックスは更新されることが多い
  * インデックスの更新は、古いインデックスの削除と挿入の2つの作業を行うため、コストが高くオーバーヘッドも増える
## 11.2 インデックスの種類
* ハッシュインデックス
* 全文検索インデックス
  * B+ツリーでは後方一致や中間一致などを上手く扱うことができない
  * 「○○という単語を含んだ行を検索したい」という時に全文検索インデックスが使える
  * 転置インデックスとして実装されている
    * 本における転置索引といえる。巻末には通常、キーワード（単語）とそのキーワードが出てくるページが記載されている
* Rツリーインデックス
  * 地図上の地点を検索するのに用いるインデックス
  * 空間インデックスとも呼ばれ、1つの平面上において、ある図形がほかの図形に含まれているか、重なるかなどの判定を行う際に使用される
* 関数インデックス
  * カラムの値に対して関数を摘要したWHERE句は、B+ツリーに用いることができない
  * 関数を摘要したクエリでもインデックスを使いたい場合に関数インデックスを使用する
* ビットマップインデックス
  * オンライン分析処理などで用いられるインデックス
## 11.3 パーティショニング
* パーティショニングとは
  * パーティショニングは、テーブルを複数のパーティションに分割し、キーの値によってどのパーティションに属する行なのかを振り分けるもの
  * パーティションはそれぞれ同じ構造を持ったテーブルとなっており、インデックスもパーティションごとに存在する
  * どのパーティションに所属するかを決めるキー(パーティションキー)を検索条件で指定すると、そのパーティションだけを検索すれば良くなるため検索効率が上がる
  * 検索対象のパーティションを絞り込む動作をパーティションの刈り込みという
* パーティショニングが適しているケース
  * 最大の判断基準は、刈り込みができる検索条件を含んだクエリの実行頻度
  * 刈り込みが有効なクエリがほどんどである場合は、パーティショニングによる処理全体の効率化が期待される
  * パーティショニングが適していても、インデックスを付ければよいケースがほどんどである
  * パーティショニングのほうが性能が向上するのは、検索対象のキーのカーディナリティが低く、大量のデータがフェッチされるとき
  * 古いデータにアクセスせず、新しいものだけを検索する場合などはレンジパーティショニングを行うことで、アクセス対象のデータの局所性を最大限に活かせる
  * アクセスの局所性が存在すると、キャッシュの効率が極めて向上し、多くの検索がメモリだけで完結しディスクI/Oを避けることができる
* パーティショニングと一意性制約
  * インデックスがパーティションごとに作成されるため（ローカルインデックス）、インデックスが所属するパーティション内では一意だが、テーブル全体では一意ではなくなる
  * パーティションごとにパーティションキーは異なるため、主キーやユニークインデックスに含めれば一意性は担保されるが、テーブルの設計上一意でなければならないキーでなくなるかもしれない
  * パーティショニングによって、一意性を保証できなくなるのであれば、パーティショニングすべきではない
* よくある勘違い
  * パーティショニングすれば、アクセスが高速化されるという間違い
  * 刈り込みをしないと意味がない
...
* トランザクションとは
  * データを正しく保つために考案された手法
  * トランザクション理論はリレーショナルモデルと異なる理論
  * 実際のアプリケーションでは、トランザクションとリレーショナルモデルの2つを同時に実践して、はじめてデータの整合性を保つことができる
* トランザクションの機能
  * 同時アクセスにより起こりうるデータの不整合を防ぐこと
  * 不測の事態にデータの整合性を保つクラッシュリカバリ
* トランザクションの鍵とスケジュール
  * 複数のクライアントからの並列実行を防ぐこと
  * しかし、並列化をしないと処理効率が良くない、性能を引き出せないなど現実的ではない
  * 上記を解決するのがスケジュールである
    * スケジュールを組み並列化する際は、個々のトランザクションを1つずつ順番に実行した時と同じ結果になることを確認する
    * スケジュールの性能を考える際は下記が重要である
      * いかに多くのトランザクションを並列化できるか
      * 最適なスケジュールを探し出すために必要なコスト
        * 現実的には、そこそこの計算コストでそこそこのスケジュールが得られるようなスケジューラーを使う
        * RDBで一般的に用いられるのはロッキングスケジューラーと呼ばれるロックを用いたもの
## 14.2 トランザクションの特徴
* ACID
  * トランザクションが満たすべき4つの性質について述べたもの
    * 原子性（Atomicity）
      * トランザクションに含まれる操作全てが成功（Commit）か失敗（Abort）になる性質
      * AbortはSQLではロールバックという
      * トランザクションが中途半端にならないこと
      * 成功、失敗の2つのステータスだけを考慮すればよい
      * Abortはロールバックが保証されているだけなので、エラー時の処理（トランザクションのリトライ）を必ずアプリケーションで実装すること
    * 一貫性（Consistensy）
      * トランザクションを実行した前後で、データの一貫性が損なわれてはならないという性質
      * トランザクション実行後にデータの変更はあるものの、データに不整合がないこと
      * トランザクションを実行するとDBはある一貫性のある状態から、別の一貫性のある状態へと遷移する
    * 分離性（Isolation）
      * 同時に実行しているj複数のトランザクションが互いに影響を与えないという性質
      * 個々のトランザクションの実行結果は、トランザクションを直列に実行した場合と結果が同じでなければならない
      * 分離性を損なうスケジュールは、スケジューラ―が事前に排除する、ロールバックされることになる
    * 永続化（Durability）
      * いったんコミットが完了したトランザクションが消失しないという性質
      * 確定したトランザクションが取り消されないこと
      * クラッシュ時もリカバリにより、データをクラッシュ前の状態まで復元できること
* さまざまな異常
  * トランザクションにあってはならない状態
    * ロストアップデート
      * ①のトランザクションが更新したデータをもとに②のトランザクションが処理を行う場合、①の更新を参照せずに更新前のデータを更新してしまうこと
      * ①の更新が失われてしまう
    * インコンシステントリード
      * あるトランザクションの実行結果が別のトランザクションの実行結果に影響を与えてしまうと、トランザクションが読み取ったデータの整合性が取れないものになる
    * ダーティリード
      * まだコミットしていないデータを読み取ることにより起きる異常
      * ①のトランザクションが更新後にまだコミットしていないデータを②のトランザクションが読み取った場合、①が失敗してしまうと②が読み取ったデータが正しいものでなくなる
    * ノンリピータブルリード
      * 1つのトランザクション内で、同じデータアイテムを複数回読み取った時、そのトランザクションが書き込みをしてないにもかかわらず値が変わってしまうこと
      * 1つのトランザクションが実行中に他のトランザクションが書き込みをしていしまうために起こる
    * ファントムリード
      * SQLのように検索範囲があるシステムに起こる問題である
      * 既存のデータアイテムによるものではなく、過去になかったデータアイテムが出現するというもの
      * ①のトランザクションが1行しか読み取っていなかったが、②のトランザクションにより新しい行が挿入された後にその行も読み取ってしまう
* スケジュールとロック
  * 異常なスケジュールが発生しないように、トランザクションのスケジュールを決定しなければならない
  * RDBにおいて最も一般的なのはロックを使い排他処理を行う方法
  * 操作対象となる行に対して、その操作を行うより前にロックをかける
  * ロックを用いることで、データの整合性を損なうスケジュールにならないよう、競合するデータアイテムへのアクセスを必要とするトランザクションをブロックする
* デッドロック
  * 順次ロックするようなアーキテクチャではデッドロックという問題に突き当たる
  * トランザクションが互いにブロックし合う状態になり、どちらのトランザクションも先に進めず行き詰ってしまう
  * デッドロックに陥ったトランザクションはロールバックする必要がある
* トランザクションの分離レベル
  * リスクや手間を回避するには、SERIALIZABLEを利用すると良い
  * リスクを取ってでも性能が欲しい場合にだけ、READ-COMMITTED、あるいはREPEATABLE-READを選択するべき
  * ダーティリードが発生してしまうREAD-UNCOMMITEDは用いられることはない
* MVCC
  * トランザクションの並列化を高めるために編み出された
  * MVCCを使うと、あるトランザクションが更新してしまったデータについて、別のトランザクションは過去の古いバージョンを参照できる
  * インコンシステントリードを防ぎつつ、並列度を高められる