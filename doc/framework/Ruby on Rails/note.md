...
## 13-2　コールバックオブジェクト
* コールバックオブジェクトとは
  * モデルのコールバックは`before_save`をはじめとした各メソッドの引数にはコールバックを実装するオブジェクトを引数に指定することができる
  * コールバックオブジェクトはモデルと独立したクラスとして定義されるため、複数のモデルの間で再利用できる
* 実装例
  * あるロジックを複数のコールバックを組み合わせて実装するコールバックオブジェクトでは、その設定がどうしても煩雑になる
  * モデルの定義内でコールバックオブジェクトが実装するコールバックと同名のクラスメソッドを全て呼び出す必要がある
  * また、コールバック尾辺ジェクトがどのコールバックを実装しているかという詳細がモデルのあちこちに散らばる
  * 上記を避けるために、コールバックオブジェクトを単体で利用せず、Concernと併用するとよい
* データ操作の実装方針
  * データの暗号化など、更新系のデータ操作（Create、Update、Delete）に関するロジックの場合、その実装にコールバックオブジェクトを利用できる
  * Concernでも実装できるが、特に理由がなければテストがしやすいコールバックオブジェクトを利用するべきである
  * データの取得などの参照系のデータ操作（Read）に関するロジックの場合、その実装にはコールバックオブジェクトを利用できない
  * モデルのコールバックは、データベースの各レコードに対応するインスタンスが初期化または更新操作されるときに実行されるものであり、参照系のデータ操作徒は関係しないためConcernを利用するべき